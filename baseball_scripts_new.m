...
f1 = @(t) pos1 + (t-t0).*vel1 + (0.5).*(t-t0).^2.*acc1;
t1 = [0:.0000001:0.3803727];
posn_f1 = f1(t1);
...
xlabel('X Trajectory'); ylabel('Y Trajectory'); zlabel('Z Trajectory');
title('Sportvision trajectory')
grid on
...
% 
% %XZ trajectory to see final position
% %add RK4 data
% hold off
% plot(posn_f1(1,:),posn_f1(3,:))
% hold on
% plot(posn_f1(1,length(posn_f1)),posn_f1(3,length(posn_f1)),'x')
% plot(posn_f2(1,:),posn_f2(3,:))
% plot(posn_f2(1,length(posn_f2)),posn_f2(3,length(posn_f2)),'x')
% grid on
% 
% xlabel('X Trajectory'); ylabel('Z Trajectory');
% title('X-Z Sportvision Trajectory of Pitch 1 and Pitch 2')
% 
% 
